...
### B. Аудит API (критично, делать сразу)
Google принудительно ставит **private** всем видео, загруженным через непроверенный API-проект. Пока аудит не пройден, ролики будут ждать в Studio, публикация одной кнопкой вручную. Заполни форму «YouTube API Services – Audit and Quota Extension»: https://support.google.com/youtube/contact/yt_api_form. Use case: «automated publishing of own channel's Shorts, single channel, no third-party users». Срок 1–3 недели. Квоты хватает: 10 000 ед./день, одна загрузка 1 600, три в день = 4 800.
...
3. После аудита: включаю `privacyStatus=public` через `publishAt`, полный автомат, ты только смотришь отчёт в TG раз в день.
