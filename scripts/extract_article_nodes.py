...
def nodes_of(path: str):
    ...
    if title:
        ...
        if re.search(r"[А-Яа-я]", text):
            seen.add(text)
            out.append(text)
    # верхняя граница 3000, а не 900: вступительные абзацы бывают длиннее 900
    # знаков, и такой абзац молча выпадал из словаря — на языковых версиях
    # оставался целый русский параграф при формально полном покрытии
    # нижняя граница 1, а не 8: короткие узлы — заголовки колонок таблиц
    # («Уровень», «Роль», «Ставка», «В год») и связки внутри ячеек — иначе
    # оставались по-русски посреди переведённой статьи на всех локалях
    for raw in re.findall(r">([^<>]{1,3000})<", body):
        text = " ".join(raw.split())
